...
describe('POST /internal/api/v1/connections/:connectionId/test', () => {
  ...
  it('should update the connection as not verified for current user', async () => {
    ...
    const response = await request(app)
      .post(`/internal/api/v1/connections/${currentUserConnection.id}/test`)
  ...
  it('should update the connection as not verified for another user', async () => {
    ...
    const response = await request(app)
      .post(`/internal/api/v1/connections/${anotherUserConnection.id}/test`)
      .set('Authorization', token)
      .expect(200);
  ...
  it('should return not found response for not existing connection UUID', async () => {
    ...
    await request(app)
      .post(`/internal/api/v1/connections/${notExistingConnectionUUID}/test`)
      .set('Authorization', token)
      .expect(404);
  });
  ...
  it('should return bad request response for invalid UUID', async () => {
    ...
    await request(app)
      .post('/internal/api/v1/connections/invalidConnectionUUID/test')
      .set('Authorization', token)
      .expect(400);
  });
});
